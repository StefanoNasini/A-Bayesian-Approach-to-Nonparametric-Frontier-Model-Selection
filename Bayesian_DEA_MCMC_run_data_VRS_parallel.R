...
progressr::with_progress(enable = TRUE, {
    ...
    foreach(kkk = runRange) %dopar%
        {
            # pbar$tick()
            out = MetropolisHansongs_z_posterior(kk = kkk, tolerance= 1.0e-07, ITER_max= 30000, ITER_min= 200, proposal_var = Proposal_var, technology = 0, H = 1)
            cat(paste0("iter is ",kkk))
