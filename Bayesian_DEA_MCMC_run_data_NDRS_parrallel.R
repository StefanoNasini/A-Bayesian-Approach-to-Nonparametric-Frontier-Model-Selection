...
setwd(paste0(getwd(),"/NDRS/",myYear))
...
progressr::with_progress(enable = TRUE, {
    ...
    foreach(kkk = runRange) %dopar%
        {
            out = MetropolisHansongs_z_posterior_NDRS(kk = kkk, tolerance= 1.0e-07, ITER_max= 30000, ITER_min= 200, proposal_var = Proposal_var, technology = 0, H = 1)
